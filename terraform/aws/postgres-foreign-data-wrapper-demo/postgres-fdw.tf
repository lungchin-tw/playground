...
locals {
  fdw_username = "fdwmock"
  fdw_db_name = "fdwmockdb"
  fdw_server = "fdwmockserver"
}

resource "aws_db_instance" "postgres_fdw" {
  depends_on = [module.network, aws_db_instance.postgres]

  identifier_prefix          = "${local.prefix}-foreign-"
  ...
  username                   = local.fdw_username
  password                   = var.db_password
  db_name                    = local.fdw_db_name
...
resource "null_resource" "fdw_setup" {
  ...
  provisioner "local-exec" {
    command = "psql postgresql://${local.fdw_username}:${var.db_password}@${aws_db_instance.postgres_fdw.endpoint}/${local.fdw_db_name} -f ${local_file.sql_script_fdw.filename}"
  }
}
